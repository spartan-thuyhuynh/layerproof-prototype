    ...
    <div className="subsidebar">
      ...
      <div className="kit-identity" ref={switchRef}>
        <button className="kit-id-row" onClick={() => setSwitchOpen((o) => !o)}>
          <div
            className="kit-id-logo"
            style={kit.symbolSrc ? { background: '#0a0a0a' } : kit.logoStyle}
          >
            {kit.symbolSrc ? (
              <img
                src={kit.symbolSrc}
        ...
        {switchOpen && (
          <div className="kit-switch-menu">
            ...
            {kits.map((k) => (
              ...
              >
                <div
                  className="kit-switch-logo"
                  style={k.symbolSrc ? { background: '#0a0a0a' } : k.logoStyle}
                >
                  {k.symbolSrc ? (
                    <img
                      src={k.symbolSrc}
            ...
            <button
              className="kit-switch-action"
              onClick={() => {
                setModal({ type: 'new' })
                setSwitchOpen(false)
              }}
            >
              <div className="kit-switch-ic">
      ...
      <nav className="subnav">
        ...
        {customCats.length > 0 && (
          <>
            <div className="subnav-group-label" style={{ marginTop: 14 }}>
              Custom
            </div>
            {customCats.map((cat: Category, idx: number) => {
              ...
              return (
                <div
                  key={cat.id}
                  className={`navitem sub custom${activeSection === cat.id ? 'active' : ''}${cat.hidden ? 'hidden-cat' : ''}`}
                >
                  <button
                    className="row grow"
                    style={{
                  ...
                  {manage && (
                    <div className="row" style={{ gap: 4 }}>
                      <button
                        className="icon-btn"
                        title="Move up"
                        onClick={() => onReorderCategory(cat.id, -1)}
                        disabled={idx === 0}
                      >
                      ...
                      <button
                        ...
                        title="Move down"
                        onClick={() => onReorderCategory(cat.id, 1)}
                        disabled={idx === customCats.length - 1}
                      >
                      ...
                      <button
                        ...
                        onClick={() => onToggleCategoryHidden(cat.id)}
                      >
